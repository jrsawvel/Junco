...
# in debug mode, app returns password created by app.
my $password;
...
# in debug mode, app returns the activation link, which has the user digest.
my $digest;
...
# ask system to create a new password.
$hr = get_new_password($test_username, $test_email);
print "\n\n HTML Response from create_new_user:\n\n" . $hr . "\n\n" if $DISPLAY_HTML_RESPONSE;

# in debug mode, app returns new password created by app.
my $new_password;
if ( $hr =~ m|new-pwd=([\w!@\$%\^&\*]+)|s ) {
    $new_password = $1;
}        
ok(defined($new_password), 'new password returned in debug mode');

# log into account
$hr = log_into_account($test_email, $new_password);
...
# this will execute the newpassword function found within loginform
sub get_new_password {
    ...
    my $mech = WWW::Mechanize->new();
    my $url = "http://$domain" . "$prog/loginform";
    $mech->get($url);
    $mech->submit_form (
        form_number => 3,
        fields => {
